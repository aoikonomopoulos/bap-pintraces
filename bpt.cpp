...
typedef boost::tuple<ADDRINT, UINT32> store_type;
typedef std::vector<store_type> stores_type;

static void process_block(buffer&, stores_type&, BBL);
static void process_instruction(buffer&, stores_type&, INS);
static void process_branching(buffer&, stores_type&, INS);
static void process_inst(buffer&, stores_type&, INS);
static void process_regs(buffer&, INS);
static void process_reads(buffer&, INS);
static void process_writes(buffer&, INS, IPOINT);
static void process_mem(buffer&, stores_type&, INS);
static void process_loads(buffer&, INS);
static void process_stores(stores_type&, INS, IPOINT);
static VOID handle_operation(buffer*,
                             stores_type*, const char*,
...
static VOID handle_stores(stores_type*, UINT32, ...);
static void flush_stores(buffer*, stores_type*);
...
struct tracer {
    static void trace(TRACE trace, visitor& out) {
        ...
        for(BBL b = TRACE_BblHead(trace);
            ...
            process_block(buff, stores, b);
        }
    }

    static void fini(INT32, visitor& out) {
        flush_stores(&buff, &stores);
        flush_buffer(out);
    }
private:
    static buffer buff;
    static stores_type stores;

    static void flush_buffer(visitor& out) {
...
static void process_block(buffer& buff, stores_type& stores, BBL b) {
    ...
    for(UINT32 i = 1, I = BBL_NumIns(b);
        ...
        process_instruction(buff, stores, ins);
    }

    INS tail = BBL_InsTail(b);
    if (INS_HasFallThrough(tail)) {
        process_instruction(buff, stores, tail);
    } else {
        process_branching(buff, stores, ins);
    }
}

static void process_instruction(buffer& buff, stores_type& stores, INS ins) {
    process_inst(buff, stores, ins);
    process_regs(buff, ins);
    process_mem(buff, stores, ins);
}

static void process_inst(buffer& buff, stores_type& stores, INS ins) {
#ifdef BPT_DEBUG
...
static void process_mem(buffer& buff, stores_type& stores, INS ins) {
    process_loads(buff, ins);
    process_stores(stores, ins, IPOINT_BEFORE);
}
...
static void process_stores(stores_type& st, INS ins, IPOINT point) {
    args_list stores;
...
static VOID handle_operation(buffer* buff,
                             stores_type* stores,
                             const char* disasm,
...
static VOID handle_stores(stores_type* stores, UINT32 args_count, ...) {
    va_list args;
    va_start(args, args_count);
    for (UINT32 i=0; i < args_count; i+=2) {
        ADDRINT addr = va_arg(args, ADDRINT);
        UINT32 size = va_arg(args, UINT32);
        store_type store = boost::make_tuple(addr, size);
        stores->push_back(store);
    }
    va_end(args);
}

static void flush_stores(buffer* buff, stores_type* stores) {
    BOOST_FOREACH(store_type store, *stores) {
        event_ptr e(new store_event(boost::get<0>(store),
                                    boost::get<1>(store)));
        buff->push_back(e);
    }
    stores->clear();
...
static void process_branching(buffer& buff, stores_type& stores, INS ins) {
    process_inst(buff, stores, ins);
    process_reads(buff, ins);
    process_loads(buff, ins);
    process_stores(stores, ins, IPOINT_BEFORE);
    if (INS_IsSyscall(ins)) {
        std::cerr << "syscall writes not traced" << std::endl;
    } else {
